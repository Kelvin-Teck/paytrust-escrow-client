...
import {
  ArrowLeft, ShieldCheck, Truck, CheckCircle2, Clock,
  AlertTriangle, DollarSign, Package, User, Calendar
} from 'lucide-react';
    ...
    <AppShell>
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        {transaction ? (
          <div className="space-y-6">
            ...
            <div className="p-8 rounded-3xl bg-white border border-slate-200 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-6">
              <div className="space-y-2">
                ...
                <h1 className="text-2xl font-bold text-slate-900">{transaction.title || 'Escrow Agreement'}</h1>
                <p className="text-xs text-slate-500">{transaction.description || 'Secured milestone-based agreement'}</p>
              </div>

              <div className="text-left md:text-right">
                <span className="text-xs text-slate-400 font-bold uppercase">Locked Escrow Total</span>
                <div className="text-3xl font-extrabold text-[#32A05F]">₦{Number(transaction.amount || 0).toLocaleString()}</div>
              </div>
            </div>
              ...
              <div className="p-6 rounded-3xl bg-white border border-slate-200 shadow-sm space-y-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-500">Provide courier dispatch details</p>
                  </div>
                </div>
                ...
                <div>
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-500">Confirm receipt of items/milestones</p>
                    </div>
                  </div>
          ...
          <div className="p-12 text-center text-slate-400">Loading escrow agreement details...</div>
        )}
      </div>
    </AppShell>
  );
